            ...
            Sub(inTable As String)
                Try
                    ...
                    If Not extExists Then
                        Log("  WARNUNG: ext." & extTable & " nicht gefunden â Ã¼bersprungen")
                        Return
                    End If

                    ' DROP _in Tabelle falls vorhanden
                    Dim dropSql As String = "IF OBJECT_ID('dbo.[" & inTable & "]','U') IS NOT NULL DROP TABLE dbo.[" & inTable & "];"
                    Log("SQL DROP:" & vbCrLf & dropSql)
                    SqlAusfuehren(connStr, dropSql, "DROP " & inTable)

                    ' SELECT INTO mit DBO Spalten aus Template
                    Dim zeilen As Integer = DatenLadenSelectInto(connStr, v, inTable, extTable, pvStr)

                    Log("  â EingefÃ¼gte Zeilen: " & zeilen.ToString())
