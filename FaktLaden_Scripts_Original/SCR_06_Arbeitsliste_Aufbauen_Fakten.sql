...
WHERE a.Status = 'ERFOLG'
  AND p.Verfahren IS NOT NULL;"
        ...
        ElseIf countFehler > 0 Then
            ' Fall 2: FEHLER vorhanden â Nur FEHLER zu AUSSTEHEND
            resetSql = "
UPDATE a
SET a.Status = 'AUSSTEHEND',
    a.LetzterSchritt = NULL,
...
WHERE a.Status = 'FEHLER'
  AND p.Verfahren IS NOT NULL;"
            resetReason = "FEHLER vorhanden â Nur FEHLER zu AUSSTEHEND (Erfolge bleiben)"
...
WHERE a.Status NOT IN ('ERFOLG', 'AUSSTEHEND')
  AND p.Verfahren IS NOT NULL;"
            resetReason = "ANDERE Status vorhanden â Alle NICHT-ERFOLG zu AUSSTEHEND"
